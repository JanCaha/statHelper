test_that("calculate_stat_models function", {
  ...
  expect_warning(
    suppressMessages(df_result <- correlations_per_group(mtcars)),
  ...
  expect_message(
    suppressWarnings(df_result <- correlations_per_group(mtcars)),
    "No grouping variable selected"
  )

  expect_warning(
    suppressMessages(df_result <- correlations_per_group(mtcars, wt)),
    "Number of variables selected for correlation calculation is `1`"
  )

  expect_message(
    df_result <- correlations_per_group(mtcars, wt, qsec),
    "No grouping variable selected"
  )
...
test_that("calculate_stat_models function errors", {
  expect_error(
    correlations_per_group(mtcars, wt, qsec, wt1, group_var = cyl),
    ".+Colum\\(s\\) `wt1` are missing from `mtcars`"
  )

  expect_error(
    correlations_per_group(mtcars, wt, qsec, group_var = cyl1),
