...
test_that("stats_numeric function fails", {
  ...
  expect_error(stats_numeric(1),
               "Variable `data` must be a `data.frame`")

  stats_to_calculate <- 1

  expect_error(stats_numeric(diamonds, stats_to_calculate = stats_to_calculate),
               "Variable `stats_to_calculate` must be a named list")

  stats_to_calculate <- list(~ n(),
                             ~ mean(.x, na.rm = TRUE))

  expect_error(stats_numeric(diamonds, stats_to_calculate = stats_to_calculate),
               "Variable `stats_to_calculate` must be a named list")

  stats_to_calculate <- list(a =~ n(),
                             b = ~ mean(.x, na.rm = TRUE),
                             c = "aa")
